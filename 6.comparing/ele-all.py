...
thetisfilenames=[
        'continuous-4cores','continuous-sediment-exner-tideforcing','discrete-4cores'
      ]
legend_names = ['计算域B','计算域B--开源边界','计算域C']
for ii,thetisfilename in enumerate(thetisfilenames):    
    ...
    if thetisfilename == 'continuous-sediment-exner-tideforcing':
...
#     axs.plot( data_time[1], thetis_elevation[0][79:-16],'g-',  label='$T_{30}$')
#     axs.set_xlabel('日期', fontsize=25,fontproperties=zw)
#     axs.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
#     axs.set_ylabel('潮位 $[m]$', fontsize=25,fontproperties=zw)
#     axs.legend(prop=zw)
