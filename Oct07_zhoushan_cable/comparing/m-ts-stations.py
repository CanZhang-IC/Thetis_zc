...
for stationname in stationnames:
    ...
    thetisfilenames=[
        'redata','redata-5min','redata-5min-nodry_wet','redata-2force'
      ]
    ...
    for (ii,thetisfilename) in enumerate(thetisfilenames):    
        ...
        thetis_velocity3=[]
        if thetisfilename=='redata-5min' or thetisfilename == 'redata-5min-nodry_wet':
            for i in range(889,1190,2):
                thetis_velocity3.append(thetis_velocity[i])
            for i in range(1801,2102,2):
                thetis_velocity3.append(thetis_velocity[i])
            for i in range(2905,3206,2):
                thetis_velocity3.append(thetis_velocity[i])
        else:
            for i in range(148,199):
                thetis_velocity3.append(thetis_velocity[i])
            for i in range(299,350):
                thetis_velocity3.append(thetis_velocity[i])
            for i in range(483,534):
                thetis_velocity3.append(thetis_velocity[i])   
        ...
        for i in range(3):
            ...
            axs[0].legend(loc='best',fontsize=14.5)
