...
BE = 'start from non-yaw'


output_dir = '../../../outputs/2.economy/discrete/intermediate_forward-step1-fromsame/non_yaw_staggered'#yaw-cable-BE-'+str(BE)[:-2]
...
if BE == 'start from non-yaw':
    ...
    result_output_dir = '../../../outputs/2.economy/discrete/intermediate-step1-fromsame/yaw-cable-BE-'+str(BE)[:-2]
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    if rank == 0:
...
### interest function###
maxoutput, maxcost = 3067.42, 2322.09

interest_functional = 0#BE/10*power_output/3000-(1-BE/10)*cablecost/maxcost

print(str(BE),str(interest_functional)+'\t'+str(power_output)+'\t'+str(cablecost)+'\t',str(iteration_numbers))
