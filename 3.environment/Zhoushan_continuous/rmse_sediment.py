...
class RMSECallback(DiagnosticCallback):
    ...
    def _cal_error(self):
        ...
        if t < self.t_initial:
            ...
            chk.close()
            # File('./original_sediment.pvd').write(original_sediment)

            # diff_vx = self.uv[0]-original_sediment[0]
            # diff_vy = self.uv[1]-original_sediment[1]
            # self.RMSE_current.append(assemble(sqrt(diff_vx**2+diff_vy**2)*dx(self.dx_index)))
            # self.RMSE_current.append(assemble(abs(dot(self.uv,self.uv)-dot(original_sediment,original_sediment))*dx(self.dx_index)))
