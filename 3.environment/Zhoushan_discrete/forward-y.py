...
output_dir = '../../../outputs/3.environment/discrete/flood_ebb-yawonly-forward/y-BE-'+str(BE)[:-2]
...
result_output_dir = '../../../outputs/3.environment/discrete/flood_ebb-yawonly/y-BE-'+str(BE)[:-2]
...
effect_two = [cb3.RMSEaverage,cb4.RMSEaverage ]
interest_functional = (1-BE/10)*power_output/maxoutput*diff_uv- sum(effect_two) * BE/10

print(str(BE),str(interest_functional)+'\t'+str(power_output)+'\t'+str(effect_two)+'\t',str(iteration_numbers))

if BE == 'start from non-yaw':
    pass
else:
    if rank ==0:
        with open('result-y.txt','a+') as f:
            f.write(str(BE)+'\t')
