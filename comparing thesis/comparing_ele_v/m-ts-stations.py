...
for stationname in stationnames:
    ...
    for i in range(3):
        axs[i].plot( otf_time[i], measured_velocity[151*i:151*(i+1):3], 'ko', label='实测数据')
    ...
    for (ii,filename) in enumerate(thetisfilenames):    
        ...
        for i in range(3):
            ...
            axs[i].plot( data_time[i], thetis_velocity3[151*i:151*(i+1)],  label=legend_names[ii])
            ...
            axs[i].set_ylabel('流速 $[m/s]$', fontsize=18,fontproperties=zw)
            axs[i].xaxis.set_tick_params(labelsize=15)
            axs[i].yaxis.set_tick_params(labelsize=15)
            axs[i].set_title('潮流测点B1处'+tidenames[i]+'潮期间流速变化',fontsize=18,fontproperties=zw)
            axs[0].legend(loc='best',prop=zw,ncol=5)
        plt.savefig('葫芦岛流速BC.png', dpi =300)
