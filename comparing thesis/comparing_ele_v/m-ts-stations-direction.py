...
for stationname in stationnames:
    ...
    for (ii,filename) in enumerate(thetisfilenames):    
        ...
        for i in range(3):
            if filename in names_30min:
                axs[i].plot( otf_time[i], thetis_direction[51*i:51*(i+1)], '.-' , label=filename)
            else:
                axs[i].plot(otf_time[i], thetis_direction[151*i:151*(i+1):3], '.-' ,label=legend_names[ii])
            
            axs[2].set_xlabel('日期', fontsize=18,fontproperties=zw)
            axs[i].set_ylabel('流向 $[°]$', fontsize=18,fontproperties=zw)
            ...
            axs[i].set_title('潮流测点B1处'+tidenames[i]+'潮期间流向变化',fontsize=18,fontproperties=zw)
            axs[2].legend(loc='lower right', prop=zw,ncol=5)
        plt.savefig('葫芦岛流向BC.png', dpi =300)
