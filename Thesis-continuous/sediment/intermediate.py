...
# namelength = len('intermediate')
# BE = float(get_index[namelength:-3])
BE = float(100)

output_dir = '../outputs/intermediate/BE'+str(BE)
...
chk = DumbCheckpoint('../prepare/breakeven_bathymetry', mode=FILE_READ)
breakeven_bathymetry = Function(P1, name='breakeven_bathymetry')
chk.load(breakeven_bathymetry)
chk.close()
# breakeven_bathymetry = Function(P1, name='breakeven_bathymetry').assign(Constant(1.0))
...
def update_forcings(t):
  with timed_stage('update forcings'):
    ...
    # tidal_v.project(v)
    elev = prepare.tidal_forcing.set_tidal_field(Function(P1),t)
    tidal_elev.project(elev) 
...
farm_options.considering_depth_related_cost = True
farm_options.breakeven_bathymetry_density = breakeven_bathymetry
...
# solver_obj.load_state(492,outputdir='../outputs/validation_5min-e&v')
...
scaled_functional = -cb.average_profit[0]
