...
output_dir = '../../../outputs/6.yaw_environment/Paper3/Zhoushan_mesh_30min/optimisation/test'#intermediate-forward-yaw-P_factor'+str(P_factor)
...
flood_dir,ebb_dir = [134.87278187487448, 92.86248175188906, 122.14135503661521, 108.22436329453366, 146.1991805255184, 94.73731366636697, 106.65062493412249, 97.2464800456186, 109.6802954631175, 105.40232323306776, 90.00000000001818, 114.80395308745605, 270.0000000033828, 323.8350089149916, 280.0533180447531, 278.8495104618326, 273.6368779049593, 287.89810907957457, 391.26528540171057, 285.1079303994894, 381.3408397067293, 280.9062909228712, 295.63390172182454, 292.20900694439035]

# result_dir = '../../../outputs/6.yaw_environment/Paper3/Zhoushan_mesh_30min/optimisation/intermediate-yaw_op-P_factor_'+str(P_factor)+'-5min_e&v'
# comm = MPI.COMM_WORLD
# rank = comm.Get_rank()
# if rank == 0:
#     def_file = h5py.File(result_dir+'/diagnostic_controls.hdf5','r+')
#     for name, data in def_file.items():
#         all_angles = list(data[-1])
#         iteration_number = len(data)
# else:
#     all_angles = None
#     iteration_number = None
# all_angles = comm.bcast(all_angles, root = 0)
# iteration_number = comm.bcast(iteration_number, root = 0)

# flood_dir, ebb_dir = all_angles[:len(farm_options.turbine_coordinates)], all_angles[len(farm_options.turbine_coordinates):]
