...
output_dir = '../../../outputs/8.final/cable-velocity-forward/'+str(BEcable)[:-2] + '-'+ str(BEsediment)[:-2] 
...
result_output_dir = '../../../outputs/8.final/cable-velocity/'+str(BEcable)[:-2] + '-'+ str(BEsediment)[:-2] 
...
if rank == 0:
    with open('result-l_y.txt','a+') as f:
        ...
        f.write(str(cb.average_power[-1])+'\t'+str(cb3.RMSEaverage)+'\t'+str(cb4.RMSEaverage)+'\t'+str(cablecost)+'\t'+str(shortestline)+'\t'+str(iteration_numbers)+'\n')
