...
output_dir = '../../../outputs/8.final/cable-sediment-forward/'+str(BEcable)[:-2] + '-'+ str(BEsediment)[:-2] 
...
E_area_centre_point_sediment = [[443385,3323260]] #behind
...
rank = comm.Get_rank()
if rank == 0:
    with open('result-l_y-sediment.txt','a+') as f:
        f.write(str(BEcable)+'\t'+str(BEsediment)+'\t')
        f.write(str(cb.average_power[-1])+'\t'+str(sediment_effect_two)+'\t'+str(cablecost)+'\n')
