...
n_layers = 4
outputdir = '../../outputs/3D/3d_v1.0_again'
lx = 500
ly = 100
nx = 160
ny = 50
...
# h_viscosity.interpolate(conditional(le(xyz[0], v_length), v_b+v_inner-xyz[0]*v_b/v_length, conditional(ge(xyz[0],lx),(xyz[0]-(lx-v_length))*v_b/v_length+v_inner,v_inner)))
h_viscosity.interpolate(conditional(ge(xyz[0],lx),(xyz[0]-(lx-v_length))*v_b/v_length+v_inner,v_inner))
File('../../outputs/3D/3d_b_v1.0/viscosity.pvd').write(h_viscosity)
...
cb = DetectorsCallback(solver_obj, locations, ['uv_2d', 'elev_2d', 'uv_3d','w_3d', 'uv_dav_2d'], name='detectors',detector_names=names)
