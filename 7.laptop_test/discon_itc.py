...
c = [Control(x) for xy in farm_options.turbine_coordinates for x in xy] +  [Control(x) for x in farm_options.individual_thrust_coefficient]
...
if 0:
    ...
    m0 =  [Constant(x) for xy in farm_options.turbine_coordinates for x in xy] + [Constant(x) for x in farm_options.individual_thrust_coefficient] 
    h0 =  [Constant(1) for xy in farm_options.turbine_coordinates for x in xy] + [Constant(0.1) for x in farm_options.individual_thrust_coefficient] 
...
if 1:
    ...
    if optimise_angle_only:
        ...
        if farm_options.considering_individual_thrust_coefficient:
            lb = list(lb) + [0.0]*len(farm_options.individual_thrust_coefficient)
