...
main_dir = '/media/can/can_disk/thetis_new'
...
with timed_stage('initialising bathymetry'):
  ...
  bathymetry2d =get_bathymetry(main_dir+'/Netcdf/bathymetry.nc', main_dir+'/Netcdf/bathymetry2.nc',mesh2d)
...
bcs = [DirichletBC(V, 0.0, 200)] #make sure this matches physicalID of open boundaries
...
with timed_stage('initialising manning'):
  ...
  manning.interpolate(Max(0.02,0.1 * (1. - u / 8e2)))
