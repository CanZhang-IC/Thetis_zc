...
# file_dir = '../../../outputs/4.yaw/Yaw_Ideal/op-conference_mesh2-5_40/f30-cos00/optimisation-aligned-both'
# file_value = h5py.File(file_dir+'/diagnostic_controls.hdf5')
...
for file_name in ['aligned-angle','aligned-both','staggered-angle','staggered-both']:
    print(file_name)
    file_dir = '../../../outputs/4.yaw/Yaw_Ideal/op-conference_mesh2-5_40/f30-cos00/optimisation-'+file_name
    file_value = h5py.File(file_dir+'/diagnostic_controls.hdf5')

    for name,value in file_value.items():
        location_angle = value[-1]
    if len(location_angle) == 12:
        print(location_angle)
    else:
        print(location_angle[24:])