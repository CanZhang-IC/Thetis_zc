...
namelength = len('closed_boundary')
...
distance = 10
speed = 2
output_dir = '../../../outputs/4.yaw/Yaw_Ideal/conference-extraforce/upwind_false/30-Force-cos0/'+str(angle)
...
# turbine_density = Function(get_functionspace(mesh2d, "CG", 1), name='density')
# radius = 10
# for coord in farm_options.turbine_coordinates:
...
#     turbine_density = turbine_density + bump/(radius**2 * unit_bump_integral)
...
def update_forcings(t):
    ...
    # n = as_vector((cos(angle),sin(angle)))
    # crossmean = Constant(assemble((uv[0]*n[1] - uv[1]*n[0])*turbine_density*dx)/4)
    # farm_options.farm_meancross.assign(crossmean)
